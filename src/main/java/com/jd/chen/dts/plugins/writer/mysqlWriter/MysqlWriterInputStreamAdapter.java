...
import com.jd.chen.dts.plugins.reader.mysqlReader.MysqlReader;
...
public class MysqlWriterInputStreamAdapter extends InputStream {
    ...
    private static Log logger = LogFactory.getLog(MysqlWriterInputStreamAdapter.class);
    ...
    private void buildString(ILine line) {
        ...
        for (int i = 0, j = 0; i < len; i++) {
            if (addFields != null && addFields[i] != null) {
                ...
                throw new DTSException("MysqlWriter: Fields number is less than column number ", JobStatus.WRITE_FAILED.getStatus(), writerID);
            }
            if (null != field) {
    ...
    private int fetchLine(byte[] buff, int off, int len) throws UnsupportedEncodingException {
        ...
        /* 该buffer已经不够放一个line，因此把line的内容保存下来，供下1~7次fetch使用
